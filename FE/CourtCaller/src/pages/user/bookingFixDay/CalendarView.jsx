...
const CalendarView = ({ selectedBranch, setSelectedBranch, onSlotSelect }) => {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      ...
      try {
        const response = await fetchPriceByBranchIDType(
          selectedBranch,
          "Fix",
          null
        ); // Update to use fetchPriceByBranchIDType
...
// fetch unavailable slot lần đầu
  ...
  return (
    <Box
      m="20px"
      sx={{ backgroundColor: "#F5F5F5", borderRadius: 2, p: 2, width: "95%" }}
    >
      <Box
        display="flex"
      ...
      >
        <FormControl
          sx={{ minWidth: 200, backgroundColor: "#0D1B34", borderRadius: 1 }}
        >
          <Typography
      ...
      {weekDays.map((day, dayIndex) => (
        <Grid container spacing={3} key={dayIndex} alignItems="center">
          {" "}
